//
//  ChangeCusInfoViewController.m
//  WelLv
//
//  Created by mac for csh on 15/5/11.
...
@interface ChangeCusInfoViewController ()

@end

@implementation ChangeCusInfoViewController
...
//填写项初始化
    ...
    //namTextFild.text = [self.chuDict objectForKey:@"to_username"];
    namTextFild.text=self.chuDict[@"to_username"];
    ...
    //zjhTextFild.text = [self.chuDict objectForKey:@"id_number"];
    zjhTextFild.text=self.chuDict[@"id_number"];
   ...
   // phoneTextFild.text = [self.chuDict objectForKey:@"phone"];
    phoneTextFild.text=self.chuDict[@"phone"];
    ...
    NSArray * arr = [NSArray arrayWithObjects:@"    请选择    ",@"身份证",@"护照",@"军官证", @"港澳通行证", @"台胞证", nil];
    NSInteger xindex =[self.chuDict[@"id_type"] integerValue];
    
  //  [[self.chuDict objectForKey:@"id_type"] integerValue];
...
// NSDictionary *parameter = @{@"to_id":to_id,@"to_username":to_username,@"phone":phone,@"idtype":idtype,@"id_number":id_number,@"sex":sex};
        ...
        NSDictionary *parameters = @{@"to_id":self.chuDict[@"to_id"],@"to_username":namTextFild.text,@"idtype":Checkstr,@"id_number":zjhTextFild.text,@"sex":self.chuDict[@"sex"],@"phone":phoneTextFild.text};
        DLog(@"%@",parameters);
        [manager POST:kChangeTravelListInfoUrl parameters:parameters
              success:^(AFHTTPRequestOperation *operation,id responseObject)
        {
                  ...
                  else
                  {
                      ...
                      {
                          [[LXAlterView sharedMyTools] createTishi:@"修改失失败"];
                      }
                  }
              }
