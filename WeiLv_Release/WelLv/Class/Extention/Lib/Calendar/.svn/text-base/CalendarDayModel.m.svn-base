...
- (BOOL)isEqualTo:(CalendarDayModel *)day
{
    ...
    //NSLog(@"is==%d",isEqual);
    //DLog(@"今====%lu,%lu,%lu",(unsigned long)self.year,(unsigned long)self.month,(unsigned long)self.day);
    //DLog(@"今1====%lu,%lu,%lu",(unsigned long)day.year,(unsigned long)day.month,(unsigned long)day.day);
    return isEqual;
}
