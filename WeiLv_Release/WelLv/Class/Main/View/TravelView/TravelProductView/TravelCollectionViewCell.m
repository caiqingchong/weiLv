...
- (void)assignValueForItemWithModel:(ProductModel *)model{
    if ([model.thumb hasPrefix:@"http"]) {
        [self.photoImage sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",model.thumb]] placeholderImage:[UIImage imageNamed:@"默认图1"]];
    }else{
        [self.photoImage sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@/%@",WLHTTP,model.thumb]] placeholderImage:[UIImage imageNamed:@"默认图1"]];
    }
    ...
    self.priceLable.text = [NSString stringWithFormat:@"  %@",model.adult_price];
}
