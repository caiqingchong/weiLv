//
//  SprintFestivalViewController.m
//  WelLv
//
...
@interface SprintFestivalViewController ()<VerticalScrollViewDelegate>
{
...
@implementation SprintFestivalViewController
...
{
    __weak SprintFestivalViewController * mianVC = self;
...
{
    __weak SprintFestivalViewController * weakSelf = self;
...
- (void)initSecondView
{
    ...
    for (int i = 0; i < lineArray.count; i++)
    {
        ...
        [[iconImageArr objectAtIndex:i / 5] addObject:[[lineArray objectAtIndex:i] stringByAppendingString:@"_新图标"]];
    }
    ...
    UIImageView * gongGaoIGV = [[UIImageView alloc] initWithFrame:CGRectMake(ViewX(gifImageView) + ViewWidth(gifImageView)-2,40/2-20/2+2, 57.5, 15)];
    gongGaoIGV.image = [UIImage imageNamed:@"微旅公告_新图标"];
    ...
    UIImageView * volumeIGV = [[UIImageView alloc] initWithFrame:CGRectMake(ViewX(line2) + ViewWidth(line2) +5,40/2-22/2,23, 24)];
    volumeIGV.image = [UIImage imageNamed:@"鼓_新图标"];
...
-(void)initSenic
{
    ...
    else
    {
        ...
        //创建中国结视图
        UIImage *zgjImage=[UIImage imageNamed:@"中国结_新图标"];
        UIImageView *zgjImageView=[[UIImageView alloc] initWithImage:zgjImage];
        
        if (IS_IPHONE_6P)
        {
             zgjImageView.frame=CGRectMake(10,2,zgjImage.size.width-1,zgjImage.size.height-1);
        }
        else
        {
            zgjImageView.frame=CGRectMake(10,3,zgjImage.size.width,zgjImage.size.height);
        }
        
        [senicView addSubview:zgjImageView];
        
        //创建周边景点视图
        UIImage *zbjdImage=[UIImage imageNamed:@"周边景点_新图标"];
        UIImageView *zbjdImageView=[[UIImageView alloc] initWithImage:zbjdImage];
        zbjdImageView.frame=CGRectMake(ViewRight(zgjImageView)+10,zgjImage.size.height/2-3,zbjdImage.size.width,zbjdImage.size.height);
        [senicView addSubview:zbjdImageView];
        
        
        
        //创建更多视图
        UIImage *moreImage=[UIImage imageNamed:@"更多_新图标"];
        UIButton *moreBtn = [UIButton buttonWithType:UIButtonTypeCustom];
        moreBtn.frame = CGRectMake(windowContentWidth-moreImage.size.width-10,zgjImage.size.height/2-3,moreImage.size.width,moreImage.size.height);
        moreBtn.backgroundColor = [UIColor clearColor];
        [moreBtn setBackgroundImage:moreImage forState:UIControlStateNormal];
         moreBtn.tag = 1;
...
-(void)SenicClick:(UIButton *)sender
{
    ...
    travelVC.area = @"city";
    travelVC.keywords = [self judgeReturnString:[model.gotoDic objectForKey:@"keyword"] withReplaceString:model.title];
    travelVC.navigationItem.title = model.title;
...
-(void)initHouserKeeperView
{
    ...
    [houseKeeperIGV setImage:[UIImage imageNamed:@"管家广告-首页"]];
...
-(void)initDestinationView
{
    ...
    if (_choiceDestListArr && _choiceDestListArr.count>0)
    {
        ...
        //创建叶子视图
        UIImage *yeImage=[UIImage imageNamed:@"叶_新图标"];
        UIImageView *yeImageView=[[UIImageView alloc] initWithImage:yeImage];
        if (IS_IPHONE_6P)
        {
            yeImageView.frame=CGRectMake(10,3,yeImage.size.width-1,yeImage.size.height-1);
        }else
        {
            yeImageView.frame=CGRectMake(10,3,yeImage.size.width,yeImage.size.height);
        }
     
        
        [destinationView addSubview:yeImageView];
        
        //创建精选旅行目的地视图
        UIImage *jxmddImage=[UIImage imageNamed:@"精选旅行目的地_新图标"];
        UIImageView *jxmddImageView=[[UIImageView alloc] initWithImage:jxmddImage];
        jxmddImageView.frame=CGRectMake(ViewRight(yeImageView)+10,yeImage.size.height/2-3,jxmddImage.size.width,jxmddImage.size.height);
        [destinationView addSubview:jxmddImageView];
        ...
        UIImage *moreImage=[UIImage imageNamed:@"更多_新图标"];
        UIButton *moreBtn = [UIButton buttonWithType:UIButtonTypeCustom];
        moreBtn.frame = CGRectMake(windowContentWidth-moreImage.size.width-10,moreImage.size.height/2+3,moreImage.size.width,moreImage.size.height);
        moreBtn.backgroundColor = [UIColor clearColor];
        [moreBtn setBackgroundImage:moreImage forState:UIControlStateNormal];
        moreBtn.tag = 3;
        [moreBtn addTarget:self action:@selector(jumpToList:) forControlEvents:UIControlEventTouchUpInside];
