...
#import "TravelCalendarLogic.h"

@interface TravelCalendarLogic ()
{
    NSDate *today;//今天的日期
    NSDate *before;//之后的日期
    NSDate *select;//选择的日期
...
@implementation TravelCalendarLogic

//计算当前日期之前几天或者是之后的几天（负数是之前几天，正数是之后的几天）
- (NSMutableArray *)reloadCalendarView:(NSDate *)date  selectDate:(NSDate *)selectdate needDays:(int)days_number customDateArray:(NSMutableArray *)array
{
    //如果为空就从当天的日期开始
    if(date == nil){
        date = [NSDate date];
...
- (void)changStyle:(CalendarDayModel *)calendarDay
{
    ...
    if(calendarSelect.year == calendarDay.year &
       ...
       calendarSelect.day == calendarDay.day){
        ...
        calendarDay.style = CellDatTypeCannotClick;
