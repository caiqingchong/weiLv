//
//  ChangeTravellerInfoViewController.m
//  WelLv
//
//  Created by 张子乾 on 16/1/28.
...
@interface ChangeTravellerInfoViewController ()
...
//存储出游人信息字典
@property (nonatomic, strong) NSDictionary *personInfoDic;


@end

@implementation ChangeTravellerInfoViewController
...
- (void)viewDidLoad {
    ...
    //出游人信息字典初始化
    self.personInfoDic = [NSDictionary dictionary];
    ...
    bgimage = [[UIView alloc] initWithFrame:CGRectMake(0, 0, [[UIScreen mainScreen] bounds].size.width, 50*4)];
...
//     NSDictionary *newInfoDic = @{@"to-id":to_idStr,@"t-name":to_username,@"t-tel":phone,@"t-zj":idtype,@"t-zjnum":id_number,@"t-sex":sex};
    ...
    //填写项初始化
    namTextFild = [[UITextField alloc ]initWithFrame:CGRectMake(ViewX(namLabel)+ViewWidth(namLabel),ViewY(namLabel),windowContentWidth - ViewX(namLabel)*2 , ViewHeight(namLabel))];
    namTextFild.placeholder = @"必填";
//    namTextFild.text = pensonDic[@"t-name"];
    [namTextFild setText:pensonDic[@"t-name"]];
    ...
    zjhTextFild = [[UITextField alloc ]initWithFrame:CGRectMake(ViewX(zjhLabl)+ViewWidth(zjhLabl),ViewY(zjhLabl),windowContentWidth - ViewX(zjhLabl)*2 , ViewHeight(zjhLabl))];
    zjhTextFild.text = pensonDic[@"t-zjnum"];
    ...
    phoneTextFild = [[UITextField alloc ]initWithFrame:CGRectMake(ViewX(phoneLabel)+ViewWidth(phoneLabel),ViewY(phoneLabel),windowContentWidth - ViewX(phoneLabel)*2 , ViewHeight(phoneLabel))];
    phoneTextFild.placeholder = @"必填";
    phoneTextFild.text = pensonDic[@"t-tel"];
    ...
    zjButton = [[UIButton alloc] initWithFrame:CGRectMake(CGRectGetMaxX(zjLabel.frame), ViewY(zjLabel), [UIScreen mainScreen].bounds.size.width-CGRectGetWidth(zjLabel.frame)*2+20, CGRectGetHeight(zjLabel.frame))];
    
    NSString *zjButtonTitleStr = nil;
    
    NSString *zjStr = [NSString stringWithFormat:@"%@",pensonDic[@"t-zj"]];
    
    if ([zjStr isEqualToString:@"1"]) {
        zjButtonTitleStr = @"身份证";
    }else if ([zjStr isEqualToString:@"2"]) {
        zjButtonTitleStr = @"护照";
    }else if ([zjStr isEqualToString:@"3"]) {
        zjButtonTitleStr = @"军官证";
    }else if ([zjStr isEqualToString:@"4"]) {
        zjButtonTitleStr = @"港澳通行证";
    }else if ([zjStr isEqualToString:@"5"]) {
        zjButtonTitleStr = @"台胞证";
    }else{
        [picker selectRow:0 inComponent:0 animated:NO];
//        [zjButton setTitle:@"身份证" forState:UIControlStateNormal];
    }
    
    [zjButton setTitle:zjButtonTitleStr forState:UIControlStateNormal];
...
- (void)loadPickerView{
    if (!pView) {
        ...
        pView =  [[UIView alloc] initWithFrame:CGRectMake(0, windowContentHeight-150-64, [[UIScreen mainScreen] bounds].size.width, 140+25)];
...
#pragma mark --------------返回上个页面的方法---------------
-(void)getBack{
    
    self.personInfoDic = @{@"name":namTextFild.text,@"credentialsNumber":zjhTextFild.text,@"credentialsStyle":zjButton.titleLabel.text};
    self.changTravelerInfoBlock(self.personInfoDic);
...
#pragma mark  ------- 修改为数字 --------
        ...
        //获取修改之前出游人的信息
        NSDictionary *personDic = self.detaillArray[personLocation-1];
        
        //生成修改后出游信息
        NSString *to_idStr = personDic[@"to-id"];
        NSString *to_username = namTextFild.text;
        NSString *idtype = Checkstr;
        NSString *id_number = zjhTextFild.text;
        NSString *sex = personDic[@"t-sex"];
        NSString *phone = phoneTextFild.text;
        NSString *member_id = [[LXUserTool sharedUserTool]getUid];
        NSString *product_category = self.product_CategoryStr;
        NSString *order_id = self.order_IdStr;
        
        NSDictionary *newInfoDic = @{@"to-id":to_idStr,@"t-name":to_username,@"t-tel":phone,@"t-zj":Checkstr,@"t-zjnum":id_number,@"t-sex":sex};
        
        //替换数组中原来的信息
        self.detaillArray[personLocation-1] = newInfoDic;
        

        
        NSString *str = [self arrayToJson:self.detaillArray];
        
        NSDictionary *parameter = @{@"to_id":to_idStr,@"to_username":to_username,@"idtype":idtype,@"id_number":id_number,@"sex":sex,@"phone":phone,@"member_id":member_id,@"product_category":product_category,@"person_info":str,@"order_id":order_id};
        
        
        [manager POST:kChangeTravelerInfoUrl parameters:parameter
