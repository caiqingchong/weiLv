...
@interface NewsMainViewController ()<VerticalScrollViewDelegate>
{
...
{
    __weak NewsMainViewController * mianVC = self;
...
{
    __weak NewsMainViewController * weakSelf = self;
...
- (void)initSecondView
{
    ...
    NSMutableArray * lineArray = [NSMutableArray arrayWithObjects:@"旅游度假",@"门票",@"邮轮",@"游学",@"签证",@"目的地参团", nil];
    NSInteger pageCount = lineArray.count / 3 < lineArray.count / 3.0 ?lineArray.count / 3 + 1 :lineArray.count / 3;
    
    if(sortSCrollVIew != nil){
        [sortSCrollVIew removeFromSuperview];
    }
    sortSCrollVIew = [[UIScrollView alloc] initWithFrame:CGRectMake(0, BannerHegit, windowContentWidth , 125*pageCount)];
    sortSCrollVIew.contentSize = CGSizeMake(0, 125*pageCount);
    sortSCrollVIew.delegate = self;
    sortSCrollVIew.pagingEnabled = NO;
    ...
    {
        [nameArr addObject:[lineArray objectAtIndex:i]];
        [iconImageArr addObject:[[lineArray objectAtIndex:i] stringByAppendingString:@"首页_"]];
    }
    
    for (int i = 0; i < lineArray.count; i++) {
        
 
        NSString * imageStr =[NSString stringWithFormat:@"%@",[iconImageArr objectAtIndex:i]];
        NSString * title =[NSString stringWithFormat:@"%@",[nameArr objectAtIndex:i]];
        UIImage *image=[UIImage imageNamed:imageStr];
        but = [[ZFJImageAndTitleButton alloc] initWithFrames:CGRectMake(20,10,image.size.width,image.size.height) shipViewCornerRadiuss:0 masksToBoundss:YES];
        
        but.iconImage.image =image;
        but.title.text =title;
        but.title.font=[UIFont systemFontOfSize:12];
        but.tag = 1000 + i;
        [but addTarget:self action:@selector(clickSortViewBtn:) forControlEvents:UIControlEventTouchUpInside];

        [sortSCrollVIew addSubview:but];
    ...
    UIView *line = [[UIView alloc] initWithFrame:CGRectMake(0, ViewHeight(sortSCrollVIew)-1, windowContentWidth, 1)];
...
- (void)initSecondView1
{
    ...
    for (int i = 0; i < lineArray.count; i++) {
        ...
        [[iconImageArr objectAtIndex:i / 4] addObject:[[lineArray objectAtIndex:i] stringByAppendingString:@"-首页"]];
    }
...
-(void)clickSortViewBtn:(UIButton *)sender
{
...
#pragma mark 周边景点
-(void)initSenic{
    float viewy = BannerHegit + 250 + 40 + 10;
...
-(void)SenicClick:(UIButton *)sender
{
    ...
    //    guoneiVC.rote_ID = -11;
    //    guoneiVC.cityTitle = model.title;
    //    guoneiVC.titles = model.title;
